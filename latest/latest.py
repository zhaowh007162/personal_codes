...
def _get_exe_command_parts():
    ...
    if argv0 and argv0.lower().endswith(".exe"):
        argv0_abs = os.path.abspath(argv0)
        if os.path.exists(argv0_abs):
            return argv0_abs, None
    ...
    if _is_windows() and exe.lower().endswith("python.exe"):
        ...
        if os.path.exists(cand):
            exe = cand
    return exe, os.path.abspath(argv0) if argv0 else ""
...
_SYMBOL_FONT_AVAILABLE = None


def _get_symbol_font(size=20, bold=True):
    """返回符号字体；同时记录是否真的找到了符号字体。"""
    global _SYMBOL_FONT_AVAILABLE
    preferred = [
        "Segoe UI Symbol",
        "Apple Symbols",
        "Symbola",
...
def _draw_app_symbol(painter, rect, font_size=24):
    """在 painter 上画程序符号。找到符号字体画 ⚙，否则画「座」。"""
    try:
        if _SYMBOL_FONT_AVAILABLE is None:
            _get_symbol_font(font_size)
        if _SYMBOL_FONT_AVAILABLE:
            painter.setFont(_get_symbol_font(font_size))
            painter.drawText(rect, Qt.AlignCenter, "⚙")
        else:
            painter.setFont(get_ui_font(font_size - 4, bold=True))
            painter.drawText(rect, Qt.AlignCenter, "座")
    except Exception:
        pass
...
class FloatingWindow(QWidget):
    ...
    def paintEvent(self, event):
        try:
            ...
            p = QPainter(self)
            p.setRenderHint(QPainter.Antialiasing, True)
            # ★ 兜底：先铺满不透明底色（Linux 无合成器时透明失效）
            if _is_linux():
                p.fillRect(self.rect(), QColor(240, 244, 248))
            p.setBrush(QBrush(QColor(25, 118, 210)))
            p.setPen(QPen(QColor(255, 255, 255), 2))
            p.drawEllipse(2, 2, w - 4, h - 4)
            p.setPen(QColor(255, 255, 255))
            _draw_app_symbol(p, self.rect(), 20)
            p.end()
        except Exception:
            pass
...
class MainWindow(QMainWindow):
    ...
    def closeEvent(self, event):
        ...
        # ★ Linux / 无托盘环境下兜底：
        # 托盘和悬浮窗都不可用，就不能隐藏，否则用户找不回主界面
        if not self._tray_available and self._floating is None:
            self._quitting = True
            try:
                if self._tray_icon is not None:
                    self._tray_icon.hide()
    ...
    def _apply_desk_preferences(self, seating):
        ...
        partner_is_desired = (
            current_partner
            and not is_empty_student(current_partner)
            and current_partner in self.desired_deskmates
        )
        partner_in_cooldown = (
            partner_is_desired and current_partner in desired_cool
        )

        if partner_is_desired and not partner_in_cooldown:
            return seating

        must_swap = partner_in_cooldown
