...
Windows 首次运行验证：
    ...
    · 加密后写入注册表 tp2
    · 验证通过写 tp3，下次跳过
    · 一次机会；输错或取消都写入 tp4，永久锁定
    · 已锁定 / 密码错误 / 取消 → 静默退出，无任何弹窗

网络检测（仅 Windows）：
    · 启动时检查网络，断网直接退出
    · 每次启动都弹「服务器通讯中」3 秒
    · 绑定期间用户按下按钮时检查网络，断网锁定
    · 验证通过后启动持续网络监控，断网立刻弹窗并关闭
...
# ★ 默认密码「12345678」的字符码形式（不能改哈希，会破坏存档逻辑）
_DEFAULT_PWD_BYTES = bytes([
    0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38
])
_DEFAULT_PWD = _DEFAULT_PWD_BYTES.decode("ascii")

# ★ 首次运行密码「12345678」的字节码 + SHA256
# 字节码形式只是防止"记事本 Ctrl+F 搜明文"
# 真正参与比对的是 SHA256 哈希，注册表 tp2 里存的是哈希加密后的值
_FIRST_RUN_PASSWORD_BYTES = bytes([
    0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38
])
_FIRST_RUN_HASH = hashlib.sha256(_FIRST_RUN_PASSWORD_BYTES).hexdigest()

# ★ Demo 版学生名单：同学01 ~ 同学44
STUDENT_NAMES = ["同学%02d" % i for i in range(1, 45)]
...
# ============================================================
#  🌐  网络检测 / 强制退出（仅 Windows 生效）
# ============================================================
def _check_network(timeout: float = NETWORK_TIMEOUT_RUNTIME) -> bool:
    """检测网络连接：仅 Windows 生效；macOS / Linux 直接返回 True"""
    if not _is_windows():
        return True
...
def _hard_exit(code: int = 1):
    """强制退出：先尝试关闭 Qt，然后 os._exit 确保立即终止"""
    try:
...
def _load_first_run_password_from_registry():
    """从注册表 tp2 读取并解密首次密码的哈希；不存在返回 None；损坏返回 None"""
    if not _is_windows():
        return None
    try:
        import winreg
...
def _write_lockout_flag_to_registry() -> bool:
    ...
    try:
        ...
        try:
            winreg.SetValueEx(
                key, REGISTRY_LOCKOUT_FLAG, 0, winreg.REG_SZ, "1"
            )
        finally:
            winreg.CloseKey(key)
        return True
...
#  ★ 服务器通讯中弹窗（3 秒自动关闭）
# ============================================================
class ServerConnectingDialog(QDialog):
    def __init__(self, parent=None, duration_ms: int = 3000):
        super().__init__(parent)
        ...
        label.setFont(get_ui_font(16, bold=True))
        label.setStyleSheet("QLabel { color: #4fc3f7; }")
        v.addWidget(label)
        sub = QLabel("请稍候")
        sub.setAlignment(Qt.AlignCenter)
        sub.setStyleSheet("QLabel { color: #b0bec5; font-size: 12px; }")
        v.addWidget(sub)
        # duration_ms 后自动关闭
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # ★ 网络监控定时器（默认不启动，验证通过后启动）
        self._network_timer = None
        self._network_failed_handling = False
        ...
        # ★ 首次运行密码验证（可能 sys.exit）
        self._check_first_run()

        # ★ 验证通过后，启动持续网络检测（仅 Windows）
    ...
    def _start_network_monitor(self):
        ...
        self._network_timer.timeout.connect(self._on_network_check)
        self._network_timer.start(NETWORK_CHECK_INTERVAL_MS)

    def _on_network_check(self):
        """定时器回调：断网立刻弹窗并退出"""
        if self._network_failed_handling:
            return
        if _check_network(timeout=NETWORK_TIMEOUT_RUNTIME):
            return
        self._network_failed_handling = True
        try:
            if self._network_timer is not None:
    ...
    def _check_first_run(self):
        """Windows 首次运行验证，非 Windows 直接返回

        流程：
          1. tp4 存在 → 已永久锁定 → 静默退出
          2. tp3 存在 → 已验证过 → 跳过
          3. 确保 tp2 存在（不存在则自动生成）
          4. 弹密码输入框（此期间断网不检查）
          5. 用户按下按钮后 → 检查网络（仅 Windows）
             · 断网 → 写入 tp4，锁定退出（弹窗提示）
          6. 输对 → 写 tp3，正常进入
          7. 输错 / 取消 → 写 tp4，静默退出（无弹窗）
        """
        if not _is_windows():
            return
...
def main():
    ...
    # ★ 启动时网络检查：断网直接退出，不启动（仅 Windows）
    if not _check_network(timeout=NETWORK_TIMEOUT_STARTUP):
        QMessageBox.critical(
            None, "无法启动",
            "服务器异常，程序无法启动。"
        )
    ...
    # ★ 每次启动都弹「服务器通讯中」3 秒（仅 Windows）
    if _is_windows():
        try:
            dlg = ServerConnectingDialog(None, duration_ms=3000)
            dlg.exec_()
        except Exception:
            pass
