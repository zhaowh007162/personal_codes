...
║  存档目录（分开放置，避免一处被全删）                            ║
║                                                                  ║
║  主存档目录：D:/JiXiu（仅 Windows 尝试）                         ║
║  回退顺序：~/.JiXiu / 程序目录/JiXiu / 当前目录/JiXiu / 临时目录 ║
║    ┌───────────────────────────────────────────────────────┐    ║
║    │  1.dat  →  管理员口令                                 │    ║
║    │  2.dat  →  偏好配置                                   │    ║
║    │  3.dat  →  上一次座位表（主）                         │    ║
...
║    │  8.dat  →  点名频率                                   │    ║
║    │  9.dat  →  上一次坐后排的学生名单                     │    ║
║    │ 10.dat  →  关注学生的最近同桌记录（主）               │    ║
║    └───────────────────────────────────────────────────────┘    ║
║                                                                  ║
║  备份目录：D:/Program Files/MMY（仅 Windows 尝试）               ║
║  回退顺序：~/.MMY / 程序目录/Program Files/MMY / 当前目录 / 临时 ║
║    ┌───────────────────────────────────────────────────────┐    ║
║    │  chart_bak.dat    →  上一次座位表（备）               │    ║
║    │  recycle_bak.dat  →  关注学生的最近同桌记录（备）     │    ║
║    │  seat_bak.dat     →  上一次坐后排名单（备）           │    ║
║    │                                                    │    ║
║    │  logs/  ← 每次「导出座位表」自动往这里写一份文字副本  │    ║
║    │          （明文 txt，文件名含时间戳，方便回溯）      │    ║
║    └───────────────────────────────────────────────────────┘    ║
╚══════════════════════════════════════════════════════════════════╝

「导出座位表」一次点击产生 3 类文件：
    ① 用户选择的路径：座位表_<时间戳>.png  （座位区截图，唯一导出到用户路径）
    ② logs 目录：座位表_<时间戳>.txt       （明文副本，只存文字，含存档码）
    ③ 备份目录：chart_bak.dat / recycle_bak.dat / seat_bak.dat（加密备份）

存档码严格记录即将备份的三项内容：
    · seating            → 对应 3.dat  / chart_bak.dat
    · last_back_students → 对应 9.dat  / seat_bak.dat
    · recent_deskmates   → 对应 10.dat / recycle_bak.dat

╔══════════════════════════════════════════════════════════════════╗
║  🗝️  教师口令的存储位置（按操作系统区分）                          ║
║                                                                  ║
║  所有系统：                                                       ║
║    6.dat —— 始终存在并保存加密后的教师口令                        ║
║                                                                  ║
║  Windows 系统（额外）：                                           ║
║    注册表路径：HKEY_CURRENT_USER\Software\SeatCrypto\JiXiu       ║
║    值名：      tp                                                ║
║    值类型：    REG_SZ（存放的是 base64 后的 AES-256-GCM 密文）    ║
║    说明：      Windows 上「双写」——6.dat 和注册表都存一份；       ║
║                程序启动时优先读注册表，读不到再回退到 6.dat。      ║
║                                                                  ║
║  删除方法（Windows）：                                            ║
║    1) 打开「注册表编辑器」（regedit）                             ║
║    2) 定位到 HKEY_CURRENT_USER\Software\SeatCrypto               ║
║    3) 删除 JiXiu 子键即可                                        ║
╚══════════════════════════════════════════════════════════════════╝

加密方案：
    · 算法：AES-256-GCM（认证加密，可检测篡改）
    · 密钥派生：PBKDF2-HMAC-SHA256（600,000 次迭代）→ 主密钥
    · 每个文件独立子密钥：HKDF-SHA256（info = "seat_file:<名称>"）
    · 每条消息随机 12 字节 nonce；salt 与 nonce 都随密文一起存储
...
RECENT_DESKMATES_BAK_FILE = os.path.join(
    BACKUP_DIR, FILE_RECENT_DESKMATES_BAK
)
...
def ensure_archive_files():
    ...
    if need_password:
        save_encrypted(PASSWORD_FILE,
                       archive.get("password", "8ik,-pl,"), salt="password")
    if need_desk:
...
class MainWindow(QMainWindow):
    ...
    # --------------------------------------------------------
    #  上次坐后排名单读写（9.dat 主 / seat_bak.dat 备）
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    #  关注学生的最近同桌记录（10.dat 主 / recycle_bak.dat 备）
    # --------------------------------------------------------
    ...
    def _load_recent_deskmates(self):
        ...
        try:
            ...
            primary_ok = False
        if primary_ok:
            self.recent_deskmates = self._parse_recent_deskmates_text(primary_text)
            return

        backup_text = None
        backup_ok = False
        try:
            backup_text = load_encrypted(RECENT_DESKMATES_BAK_FILE, salt="recent_bak")
            if backup_text:
                backup_ok = True
        except ValueError:
            backup_ok = False
        if not backup_ok:
            return
        self.recent_deskmates = self._parse_recent_deskmates_text(backup_text)
        try:
    ...
    def _load_recent_deskmates_bak(self):
        ...
        try:
            ...
            if text:
                self.recent_deskmates_bak = self._parse_recent_deskmates_text(text)
        except ValueError:
            pass
    ...
    def _save_recent_deskmates_bak(self):
        ...
        payload = "\n".join(lines)
        try:
            save_encrypted(RECENT_DESKMATES_BAK_FILE, payload, salt="recent_bak")
        except Exception:
            pass
    ...
    # ★ Union 构造（按文件存在性，依次遍历四部分）
    #   第一类：上次坐后排名单
    #     第一部分：9.dat 存在        → Union += 9.dat
    #     第二部分：9.dat 不存在      → Union += seat_bak.dat
    #   第二类：最近同桌记录
    #     第三部分：10.dat 存在       → Union += 10.dat
    #     第四部分：10.dat 不存在     → Union += recycle_bak.dat
    def _union_banned_names(self):
        union = set()
        ...
        if os.path.exists(LAST_BACK_FILE):
            ...
            # 第二部分：9.dat 不存在 → seat_bak.dat
            for name in self.last_back_students_bak:
                if name and not is_empty_student(name):
                    union.add(name)

        # ---- 第二类：最近同桌记录 ----
        if os.path.exists(RECENT_DESKMATES_FILE):
            # 第三部分：10.dat 存在
            for name in self.recent_deskmates[-DESK_COOLDOWN_TIMES:]:
                if name and not is_empty_student(name):
                    union.add(name)
        else:
            # 第四部分：10.dat 不存在 → recycle_bak.dat
            for name in self.recent_deskmates_bak[-DESK_COOLDOWN_TIMES:]:
                if name and not is_empty_student(name):
                    union.add(name)
    ...
    # ★ 同桌配对
    #   Union 见 _union_banned_names（9.dat / seat_bak.dat / 10.dat / recycle_bak.dat）
    #   全集 = 全体意愿学生（desired_deskmates）
    #   补集 = 全集 - Union
    #   若补集非空 → 从补集中随机抽一个意愿学生
    #   若补集为空 → 从非意愿名单里随机抽一个（排除 Union）
    #   ★ 交换前校验 1：被换走的 current_partner 若会被换到后排
    #     且他上轮坐过后排 → 跳过本次交换
    #   ★ 交换前校验 2：被换来的 chosen 若会被换到后排
    #     且他上轮坐过后排 → 跳过本次交换
    def _apply_desk_preferences(self, seating):
        ...
        union = self._union_banned_names()

        if (current_partner in self.desired_deskmates
                and current_partner not in union):
            return seating
        ...
        complement = [
            ...
            if n and not is_empty_student(n) and n not in union
        ]
        # 只保留在座的学生
        complement = [n for n in complement if n in seating.values()
        ...
        # ★ 交换前校验 1：被换走的 current_partner 会被放到 chosen_pos
        #   如果 chosen_pos 是后排 且 current_partner 上轮坐过后排 → 跳过
        if (current_partner
                and is_back_seat(chosen_pos)
                and current_partner in self.last_back_students):
            return seating

        # ★ 交换前校验 2：被换来的 chosen 会被放到 other_pos
        #   如果 other_pos 是后排 且 chosen 上轮坐过后排 → 跳过
        if (chosen
    ...
    def import_seating(self):
        ...
        if not code_last_back:
            # 存档码未带 9.dat，从 code_seating 反推
            for pos, name in code_seating.items():
                if is_back_seat(pos) and not is_empty_student(name):
                    code_last_back.add(name)

        # 10.dat（recent_deskmates）
        code_recent = []
        rd = archive.get("recent_deskmates")
        if isinstance(rd, list):
            for n in rd:
                if not isinstance(n, str):
                    ...
                    code_recent.append(n)
        while len(code_recent) < DESK_COOLDOWN_TIMES:
            code_recent.insert(0, "")
        code_recent = code_recent[-DESK_COOLDOWN_TIMES:]
        ...
        # 覆盖 10.dat
        self.recent_deskmates = code_recent
        self._save_recent_deskmates()

        self.statusBar().showMessage("已完成。", 3000)

    # ★ 从座位表 txt 中提取存档码并解密
    #   成功 → 返回 dict（含 seating / last_back_students / recent_deskmates）
    #   失败 → 返回 None
    ...
    # --------------------------------------------------------
    #  导出座位表（只导出图片到用户选择位置；文字副本固定写 MMY/logs）
    # --------------------------------------------------------
    def export_seating(self):
        ...
        # ★ 存档码严格记录：3.dat（seating）+ 9.dat（last_back）+ 10.dat（recent）
        snapshot = {
            "v": 3,
