...
def _get_exe_command_parts():
    """返回 (exe, script_or_None)。
    script 为 None：单 exe 直接启动。
    script 为字符串：exe + script（源码运行）。

    优先级：
      1) argv[0] 是 .exe → Nuitka onefile / PyInstaller onefile
         直接用 argv[0]，它是用户实际双击的原始 exe 路径，
         不受 Nuitka onefile 临时解压目录影响。
      2) sys.frozen → PyInstaller standalone
      3) 兜底 → 源码运行，用 pythonw.exe + 脚本路径
    """
