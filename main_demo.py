...
def main():
    ...
    # ★ 单实例检测：
    #   - 拿锁失败 → 通知已运行实例显示主界面 → 自己退出
    #   - 拿锁成功 → 正常启动，并启动 IPC server 接受后续唤起
    ...
    # 启动 IPC server，供后续实例唤起主界面
    _start_single_instance_server(win._show_main)

    # ★ 无论首次还是后续启动，只要用户没手动关闭自启，就写入
    # （幂等：每次启动都写一遍，防止用户手动删除注册表 / plist / desktop）
    if win.autostart_pref != "false":
        _enable_autostart()

    if win._is_first_run:
        # 首次运行：主界面正常显示；不做托盘化 / 悬浮窗
        win.show()
    else:
        # 后续启动：主界面隐藏；托盘 + 悬浮窗常驻
        win.show()
